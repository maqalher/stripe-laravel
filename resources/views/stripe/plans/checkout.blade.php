            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('plans.process')}}" method="POST" id="subscribe-form">
                        @csrf
                        <span class="mb-3"><b>Your Subscription is</b> {{strtoupper($plan->name)}}</span> <span style="float:right"> $ {{$plan->price/100}}</span>

                        <input type="hidden" name="plan_id" value="{{$plan->plan_id}}" class="form-control"> <br>

                        <label for="card-holder-name mt-3">Card Holder Name</label>
